...
class Dispatch
{
    ...
    {
        $dir = $config['__APP__'] . PLATFORM . '/' . 'controllers';
        //判断模块是否存在
        if(!is_dir($dir)){
            //抛出异常
        ...
        //实例化控制器类
        $controller_name = APP_NAMESPACE_NAME . '\\' . PLATFORM  . '\controllers\\' . CONTROLLER;
        $filename = $config['__APP__'] . PLATFORM . '/' . 'controllers' . '/' . CONTROLLER  . EXT;
